...
public class SalesmanViewBL {
    ...
    public List<Product> getStore(long id) {
        Salesman salesman = catalogue.keySet().stream().filter(s -> s.getId() == id).collect(Collectors.toList()).get(0);
    ...
    private static Salesman retrieveSalesmanFromCatalogue(long id) {
        return catalogue.keySet().stream().filter(s -> s.getId() == id).collect(Collectors.toList()).get(0);
    }
